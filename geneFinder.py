...
def findBackGenes(depth, LineNum, lines):
    ...
    for i in range (LineNum, -1, -1):
        if depth < 0 :
            ...
            break
        if (re.search('/product="hypothetical protein"', lines[i]) != None):
            genes.insert(0, [(fieldNum),'hypo'])
            depth = depth-1
            fieldNum = fieldNum-1
...
def findforwardGenes(x, LineNum, LinesLen, lines):
    pattern = r'/gene="([^"]+)"'
    fieldNum = 1
...
def writeToCSV(genes, CSV):    
    fields = ['filename'] + list(range(-10, 11))
...
while (gene != 'exit'):
    ...
    depth = -1
    while (10<depth or depth<0):
        depth = int(input("Input search depth (1-10): "))
    instances = 0
    filesSearched = 0
    fileNum = len(files)
